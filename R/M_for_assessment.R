...
# Setup ----
libs <- c("tidyverse", "janitor", "readr", "rstan", "fishmethods", "truncnorm", "shiny")
...
Then_M<-function(Amax) #from Cope NMT 
{
  M_vals<-rep(NA,3)
...
#from github.com/JaneSullivan-NOAA (note, location will change once finalized)

urlfile_raw <- "https://raw.githubusercontent.com/JaneSullivan-NOAA/rockfishM/main/results/2021/M_estimates.csv"
urlfile_formatted <- "https://raw.githubusercontent.com/JaneSullivan-NOAA/rockfishM/main/results/2021/formatted_M_results.csv"
...
Mup_form <- read_csv(url(urlfile_formatted)) %>% 
  select(!M_estimate)
data_input <- unique(Mup_form$data_input)

M_meth2 <- as.data.frame(cbind(M_method, data_input))

Mup_raw <- Mup_raw %>% 
  left_join(M_meth2, by = 'M_method') %>% 
  left_join(Mup_form, by = c('species', 'area', 'version', 'data_input'))

OROX_Mup <- Mup_raw %>% 
  filter(species %in% spec_cmn) %>% 
  mutate(unqID = paste(species, data_input_values, sep = "")) %>% 
  filter(unqID != "harlequin rockfish81", #remove these two, they are not official ages
         unqID != "harlequin rockfish72") %>% 
  select(-unqID)

# Current assessment M values ----
Mspecs <- unique(OROX_Mup$species)
...
# 3) downweight by 0.5 if assumed to be biased
# 2) downweight by 0.5 if borrowed from another region
# 3) downweight by 0.25 if input data are highly uncertain (e.g., dry weight/temp)
# 4) downweight by 0.5 if reliant on M/K ratio

Mwt <- OROX_Mup %>% 
  select(species, M_method, area, data_input_values, references)
