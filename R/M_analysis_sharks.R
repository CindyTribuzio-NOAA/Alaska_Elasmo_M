...
out_path <- paste0("results/sharks")
dir.create(out_path)
...
lh <- read_sheet("https://docs.google.com/spreadsheets/d/1cj5mtrbQ4w2GCJTK-mRY-rKKBnhm2e1yedlw9KMfXuo/edit?gid=0#gid=0",
  ...
  filter(species %in% c('spiny_dogfish', 'blue_shark', 'pacific_sleeper_shark', 'salmon_shark'))
...
tmax <- read_csv(paste0(getwd(), '/results/tmax_summary.csv')) %>% 
  filter(Species %in% c('spiny dogfish', 'blue shark', 'pacific sleeper shark', 'salmon shark')) %>% 
...
write_csv(fullout, paste0(out_path, "/l_M_estimates_sharks.csv"))
l_fullout <- fullout
...
write_csv(as.data.frame(fullout), paste0(out_path, "/M_estimates_sharks.csv"))
...
summls %>% 
  writexl::write_xlsx(path = paste0(out_path, '/M_species_tables_sharks.xlsx'))
...
summ %>% 
  ...
  write_xlsx(path = paste0(out_path, '/M_species_tables_full_sharks.xlsx'))
...
l_fullout <- l_fullout %>% 
  ...
  mutate(input_area = factor(input_area,
                             labels = c('NWP', 'GOA', 'BS', 'AI', 'BC', 'WC', 'multi_region'),
                             levels = c('NWP', 'GOA', 'BS', 'AI', 'BC', 'WC', 'multi_region'),
                             ordered = TRUE))
